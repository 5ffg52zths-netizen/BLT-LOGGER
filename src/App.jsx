...
const DEFAULT_ROSTER = ["KTRよ", "よる", "みやや", "たぁ", "KaNTa", "SINRA", "バチンウニ", "Min", "ぽよ", "きのぴ", "ほいぱ", "ごはんおいSEA", "こっこ♪", "こーすけ", "たけのこ", "まり", "ゆいん", "ゆうき", "トマホーク", "プリ", "きょりゅこ"];
...
async function loadRoster() {
  try {
    ...
    if (Array.isArray(r)) {
      const merged = Array.from(new Set([...DEFAULT_ROSTER, ...r.filter(n => typeof n === "string" && n.trim())]));
      ROSTER = merged;
      try { await sSet(KEY.roster, { v: 2, names: merged }); } catch (e) {}
...
async function generateGrowthReport(sessions, onProgress) {
  if (sessions.length < 2) return null;
  const timeline = buildGrowthTimeline(sessions);
  const tl = JSON.stringify(timeline, null, 1);
  ...
  const playerNames = [...new Set(timeline.flatMap(t => Object.keys(t.players || {})))];
...
async function generateAllLegends(sessions, onProgress) {
  const timeline = buildGrowthTimeline(sessions);
...
const OldBookBg = memo(function OldBookBg() {
  return (
    <div aria-hidden style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", background: "linear-gradient(180deg,#0a0703,#0f0a04 40%,#080502)", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", left: "-25%", right: "-25%", top: 0, height: "60%", animation: "bltLamp 9s ease-in-out infinite", background: "radial-gradient(ellipse 26% 30% at 50% 22%, rgba(255,222,160,0.34), transparent 70%)", filter: "blur(3px)" }} />
      {/* 吊り下げ電灯: 天井からのコード + 電球。ヘッダーの下(約90px)にぶら下げて見えるようにする */}
      <div style={{ position: "absolute", left: "50%", top: 0, width: "2px", height: "86px", transform: "translateX(-50%)", background: "linear-gradient(180deg, rgba(120,95,60,0.0) 20%, rgba(150,120,80,0.55))" }} />
      <div style={{ position: "absolute", left: "50%", top: "86px", width: "13px", height: "13px", transform: "translateX(-50%)", borderRadius: "50%", background: "radial-gradient(circle at 42% 36%, #fff2d6, #ffcf8a 60%, #d9a95f)", boxShadow: "0 0 18px 7px rgba(255,208,130,0.65), 0 0 60px 26px rgba(255,190,110,0.22)", animation: "bltLamp 9s ease-in-out infinite" }} />
...
const MuseumBg = memo(function MuseumBg() {
  return (
    <div aria-hidden style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", background: "linear-gradient(180deg,#0b130c,#101a0e 55%,#131208)", overflow: "hidden" }}>
      {/* 壁紙: 森緑のダマスク風の淡い縦帯 */}
      <div style={{ position: "absolute", inset: 0, background: "repeating-linear-gradient(90deg, rgba(120,160,90,0.05) 0 34px, transparent 34px 68px)" }} />
      {/* ピクチャーライト: 上からの真鍮色の光 */}
      <div style={{ position: "absolute", left: "-15%", right: "-15%", top: "-120px", height: "320px", background: "radial-gradient(ellipse at center, rgba(217,166,72,0.14), transparent 62%)", filter: "blur(8px)" }} />
      {/* 木の腰板: 下部のパネル */}
      <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, height: "26vh", background: "linear-gradient(180deg, rgba(60,42,18,0.55), rgba(38,26,10,0.85)), repeating-linear-gradient(90deg, rgba(0,0,0,0.24) 0 2px, transparent 2px 64px)", borderTop: "2px solid rgba(150,110,55,0.4)", boxShadow: "0 -6px 18px rgba(0,0,0,0.45)" }} />
      {/* 部屋のビネット */}
      <div style={{ position: "absolute", inset: 0, boxShadow: "inset 0 0 120px 30px rgba(0,0,0,0.7)" }} />
    </div>
  );
});
...
function Toast({ toast, onDismiss }) {
  ...
  return (
    <div style={{ position: "fixed", top: "16px", left: "50%", transform: "translateX(-50%)", zIndex: 500, background: "#1a1228", border: `1px solid ${col}66`, borderRadius: "10px", padding: "10px 18px", color: col, fontSize: "13px", fontWeight: 600, boxShadow: "0 8px 24px rgba(0,0,0,0.5)", maxWidth: "90%", textAlign: "center" }}>
      {toast.message}
    </div>
  );
}
...
function ConfirmDialog({ title, message, confirmLabel, danger, onConfirm, onCancel }) {
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 400, background: "rgba(0,0,0,0.85)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
...
function MatchEditModal({ match, onSave, onClose, roster }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.9)", backdropFilter: "blur(6px)", display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
...
function WeaponAssignModal({ images, onApply, onSync, onClose, weapons }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 320, background: "rgba(0,0,0,0.92)", backdropFilter: "blur(6px)", display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
...
function AddSessionModal({ onClose, onSave, onToast, sessions }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.88)", backdropFilter: "blur(6px)", display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
...
function DatePage({ sessions, onAdd, onUpdateReview, onDeleteSession, onToast, onChangeMVP, onRefreshSession }) {
  ...
  return (
    <div>
      {sorted.length === 0 && <div style={{ textAlign: "center", color: C.muted, padding: "40px 0", fontSize: "14px" }}>まだデータがありません。右下の歯車から読み込んでください</div>}
  ...
  if (sessions.length === 0) {
    return (
      <div style={{ textAlign: "center", color: C.muted, padding: "60px 20px", fontSize: "14px" }}>
        <div style={{ fontSize: "40px", marginBottom: "12px" }}><Ico e="🦑" /></div>
        まだデータがありません。右下の歯車からオーナーの共有データを読み込むと、ここに今夜のMVPが輝きます。
      </div>
    );
  }
...
function SessionTrendChartImpl({ sessions }) {
  ...
  return (
    <div style={{ marginTop: "14px" }}>
      <div style={{ fontSize: "10px", color: C.cyan, fontFamily: "Rajdhani, sans-serif", letterSpacing: "0.08em", marginBottom: "5px" }}><Ico e="📈" /> セッション毎の成績推移（1試合あたり平均）</div>
...
function StatsPageImpl({ sessions, growth, growthLoading, onRefreshGrowth }) {
  ...
  return (
    <div>
      <GrowthReportCard report={growth} loading={growthLoading} onRefresh={onRefreshGrowth ? () => onRefreshGrowth(sessions, { force: true }) : null} refreshing={growthLoading} />
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "12px", padding: "14px", marginBottom: "16px" }}>
...
function ViewerDataModal({ onClose, onImported, onToast }) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
...
function ShareModal({ sessions = [], onClose, onImported, onToast }) {
  ...
  const commitRename = async () => {
    ...
    if (v !== roster[editIdx] && roster.includes(v)) { onToast({ type: "error", message: "その名前は既に登録されています" }); return; }
    const next = roster.map((n, i) => i === editIdx ? v : n);
    if (await persistRoster(next, "名前を変更しました")) cancelRename();
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.88)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-end" }}>
      <div style={{ width: "100%", boxSizing: "border-box", maxHeight: "92vh", overflowY: "auto", overflowX: "hidden", background: "#110a22", borderRadius: "20px 20px 0 0", border: `1px solid ${C.border}`, borderBottom: "none", padding: "20px", display: "flex", flexDirection: "column", gap: "16px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          ...
          {roster.length > 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: "5px", marginTop: "2px", maxHeight: "260px", overflowY: "auto" }}>
              {roster.map((n, i) => (
                <div key={n + i} style={{ display: "flex", alignItems: "center", gap: "6px", background: "rgba(255,255,255,0.04)", border: `1px solid ${C.border}`, borderRadius: "10px", padding: "6px 8px 6px 12px" }}>
                  {editIdx === i ? (
                    <>
                      <input autoFocus value={editValue} onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === "Enter") commitRename(); if (e.key === "Escape") cancelRename(); }} style={{ flex: 1, background: "rgba(255,255,255,0.06)", border: `1px solid ${C.cyan}55`, borderRadius: "8px", padding: "6px 10px", color: C.text, fontSize: "13px", outline: "none" }} />
                      <button onClick={commitRename} disabled={rosterBusy} style={{ ...B, background: C.green + "18", border: `1px solid ${C.green}44`, color: C.green, padding: "6px 10px", fontSize: "12px" }}>保存</button>
                      <button onClick={cancelRename} style={{ ...B, background: "transparent", border: `1px solid ${C.border}`, color: C.muted, padding: "6px 10px", fontSize: "12px" }}>取消</button>
                    </>
                  ) : (
                    <>
                      <span style={{ flex: 1, fontSize: "14px", color: C.text, fontFamily: "Rajdhani, sans-serif", fontWeight: 600 }}>{n}</span>
                      <button onClick={() => startRename(i)} disabled={rosterBusy} style={{ ...B, background: "transparent", border: "none", color: C.cyan, fontSize: "14px", padding: "2px 6px" }} title="名称変更"><Ico e="✏️" /></button>
                      <button onClick={() => removePlayer(n)} disabled={rosterBusy} style={{ ...B, background: "transparent", border: "none", color: C.pink, fontSize: "16px", padding: "0 6px", lineHeight: 1 }} title="削除">×</button>
                    </>
                  )}
                </div>
              ))}
          ...
          <div style={{ fontSize: "10px", color: C.muted }}>登録メンバー {roster.length}名</div>
        </div>
...
// チーム分け。mode="random"は完全ランダム、"balance"は実力スコアの蛇順(1位A,2位B,3位B,4位A...)で戦力を均す。
function splitTeams(names, mode, scoreOf) {
  const playing = names.slice(0, Math.min(8, names.length));
  if (mode === "balance") {
    const sorted = playing.slice().sort((a, b) => (scoreOf(b) || 0) - (scoreOf(a) || 0));
    const A = [], B = [];
    sorted.forEach((n, i) => ((i % 4 === 0 || i % 4 === 3) ? A : B).push(n));
    return { A, B };
  }
  const sh = seededShuffle(playing, Math.random);
  return { A: sh.slice(0, Math.ceil(sh.length / 2)), B: sh.slice(Math.ceil(sh.length / 2)) };
}
...
function MenuPageImpl({ onOpen }) {
  const tiles = [
    ...
    { id: "studio", icon: Rat, label: "ねずみスタジオ", desc: "思い出のギャラリー", c: "#d9a648" },
  ];
  return (
    <div style={{ height: "calc(100dvh - 132px)", minHeight: "430px", display: "grid", gridTemplateColumns: "1fr 1fr", gridTemplateRows: "repeat(4, 1fr)", gap: "10px", overflow: "hidden" }}>
      {tiles.map((t, i) => (
...
function KawarabanPageImpl({ sessions, growth, onGenerate, progress }) {
  ...
  const gen = async () => { if (loading) return; setLoading(true); try { await onGenerate(); } finally { setLoading(false); } };
  return (
    <div>
      <div style={{ textAlign: "center", marginBottom: "14px" }}>
...
function MatchingPageImpl({ sessions }) {
  ...
  // 実力指標 = これまでの平均キル数
  const scoreOf = (name) => { const p = (an.players || {})[name]; if (!p || !p.games) return 0; return p.kG ? p.kills / p.kG : p.kills / p.games; };
  ...
  const doSplit = (m, members) => {
    const t = splitTeams(members || playing, m, scoreOf);
  ...
  const dealWeapons = () => {
    ...
    const deck = seededShuffle(WEAPON_DATA, Math.random).slice(0, playing.length);
  ...
  const spin = () => {
    ...
    const iv = setInterval(() => {
      setRule(MATCH_RULES[Math.floor(Math.random() * MATCH_RULES.length)]);
    ...
    const to = setTimeout(() => {
      ...
      const fr = rPool[Math.floor(Math.random() * rPool.length)];
      const fs = sPool[Math.floor(Math.random() * sPool.length)];
      ruleHist.current = [fr, ...ruleHist.current].slice(0, 2);
    ...
    <div style={{ flex: 1, background: `linear-gradient(160deg,${color}14,rgba(255,255,255,0.02))`, border: `1.5px solid ${color}66`, borderRadius: "14px", padding: "0 12px 12px", overflow: "hidden", boxShadow: `0 0 14px ${color}14, 0 8px 20px rgba(0,0,0,0.4)` }}>
      ...
      {names.map((n, i) => (
        <div key={n} style={{ background: "rgba(0,0,0,0.25)", borderRadius: "9px", padding: "8px 10px", marginBottom: "6px", animation: `bltFadeUp 0.3s ease-out ${i * 0.08}s backwards` }}>
          <div style={{ fontSize: "13px", fontWeight: 700, color: C.text }}>{n}</div>
  ...
  return (
    <div>
      ...
      <div style={{ background: `linear-gradient(160deg,${C.yellow}0c,rgba(255,255,255,0.02))`, border: `1px solid ${C.yellow}33`, borderRadius: "14px", padding: "14px", textAlign: "center" }}>
        ...
        <div style={{ fontSize: "9px", color: C.muted, marginTop: "8px" }}>※ 直近2回と同じルール・ステージは出ません</div>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      {players.length === 0 && !growthLoading && (
        <div style={{ textAlign: "center", color: C.muted, fontSize: "12px", padding: "30px 0", lineHeight: 1.8 }}>
          ...
          「コーチング分析を更新」を押すと、<br />専属コーチが全員分のレポートを書き上げます。
        </div>
      )}
      {players.map((ai, i) => {
        const col = acc(i);
...
function StudioPageImpl({ onToast }) {
  ...
  const FRAME_BG = "linear-gradient(135deg,#9a7a45,#e0bd75 28%,#8a6a3b 62%,#caa45e)";
  return (
    <div>
      <div style={{ textAlign: "center", marginBottom: "12px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}>
          <Rat size={22} color="#d9a648" strokeWidth={1.8} style={{ filter: "drop-shadow(0 0 6px rgba(217,166,72,0.5))" }} />
          <div style={{ fontSize: "20px", fontWeight: 900, letterSpacing: "0.18em", color: "#e6c078", textShadow: "0 0 16px rgba(217,166,72,0.35)" }}>ねずみスタジオ</div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "18px 16px", padding: "12px 4px 8px", background: "radial-gradient(circle at 50% 0%, rgba(140,105,50,0.07), transparent 65%)", borderRadius: "14px" }}>
        {(items || []).map((it, i) => (
          <button key={it.id} onClick={() => setViewer(it.id)} style={{ ...B, padding: 0, background: "transparent", borderRadius: "6px", transform: `rotate(${studioRot(it.id)}deg)`, animation: `bltFadeUp 0.35s ease-out ${Math.min(i, 8) * 0.05}s backwards` }}>
            <div style={{ padding: "9px", borderRadius: "6px", background: FRAME_BG, boxShadow: "0 10px 22px rgba(0,0,0,0.55), inset 0 0 0 1px rgba(58,40,12,0.65)" }}>
              <div style={{ background: "#f2ead6", padding: "7px 7px 6px", boxShadow: "inset 0 0 8px rgba(0,0,0,0.28)" }}>
                {imgs[it.id]
                  ? <img src={imgs[it.id]} alt="" style={{ display: "block", width: "100%", aspectRatio: "16 / 9", objectFit: "cover", border: "1px solid rgba(0,0,0,0.35)" }} />
                  : <div style={{ width: "100%", aspectRatio: "16 / 9", background: "rgba(0,0,0,0.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "10px", color: "#8a7a5a" }}>読み込み中…</div>}
                <div style={{ margin: "7px auto 0", width: "fit-content", background: "linear-gradient(180deg,#d3b060,#8f7430)", color: "#33260e", fontSize: "8.5px", fontWeight: 700, padding: "2px 10px", borderRadius: "3px", letterSpacing: "0.12em", boxShadow: "inset 0 1px 0 rgba(255,255,255,0.35)" }}>{fmtAt(it.at)}</div>
              </div>
            </div>
          </button>
        ))}
      ...
      {viewer && imgs[viewer] && (
        <div onClick={() => setViewer(null)} style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(4,3,8,0.92)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "16px", backdropFilter: "blur(6px)" }}>
          <div onClick={e => e.stopPropagation()} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} style={{ maxWidth: "560px", width: "100%", padding: "12px", borderRadius: "8px", background: FRAME_BG, boxShadow: "0 20px 50px rgba(0,0,0,0.7)" }}>
            <div style={{ background: "#f2ead6", padding: "10px" }}>
              <img src={imgs[viewer]} alt="" style={{ display: "block", width: "100%", maxHeight: "60vh", objectFit: "contain", border: "1px solid rgba(0,0,0,0.35)", background: "#141414" }} />
              <div style={{ margin: "8px auto 0", width: "fit-content", background: "linear-gradient(180deg,#d3b060,#8f7430)", color: "#33260e", fontSize: "10px", fontWeight: 700, padding: "3px 14px", borderRadius: "3px", letterSpacing: "0.14em" }}>{fmtAt(((items || []).find(x => x.id === viewer) || {}).at)}</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: "10px", marginTop: "14px", alignItems: "center" }} onClick={e => e.stopPropagation()}>
            <button onClick={() => goView(-1)} disabled={viewIdx <= 0} aria-label="前の写真" style={{ ...B, width: "40px", height: "40px", borderRadius: "50%", background: "rgba(255,255,255,0.08)", border: `1px solid ${C.border}`, color: viewIdx <= 0 ? C.muted : "#e6c078", display: "flex", alignItems: "center", justifyContent: "center", opacity: viewIdx <= 0 ? 0.4 : 1 }}><ChevronLeft size={20} /></button>
            ...
            <button onClick={() => goView(1)} disabled={viewIdx < 0 || viewIdx >= (items || []).length - 1} aria-label="次の写真" style={{ ...B, width: "40px", height: "40px", borderRadius: "50%", background: "rgba(255,255,255,0.08)", border: `1px solid ${C.border}`, color: (viewIdx < 0 || viewIdx >= (items || []).length - 1) ? C.muted : "#e6c078", display: "flex", alignItems: "center", justifyContent: "center", opacity: (viewIdx < 0 || viewIdx >= (items || []).length - 1) ? 0.4 : 1 }}><ChevronRight size={20} /></button>
          </div>
        </div>
      )}
...
function AppInner() {
  ...
  const refreshGrowth = useCallback(async (sessions, opts = {}) => {
    ...
    try {
      const report = await generateGrowthReport(sessions, (t) => dispatch({ type: "GROWTH_PROGRESS", text: t }));
      if (report) {
  ...
  const handleGenerateLegends = useCallback(async () => {
    try {
      const legends = await generateAllLegends(state.sessions, (t) => dispatch({ type: "GROWTH_PROGRESS", text: t }));
  ...
  // 起動時の自動更新: 保存済みURL(data.txt / studio.txt)があれば毎回最新を取得して取り込む。
  // 失敗しても手元のキャッシュで表示は続く(オフラインでも見られる)。
  useEffect(() => {
    (async () => {
      try {
        const urlData = localStorage.getItem(VIEWER_URL_DATA_KEY) || "";
        const urlStudio = localStorage.getItem(VIEWER_URL_STUDIO_KEY) || "";
        if (urlData) {
          const text = await fetchShareText(urlData);
          if (text) {
        ...
        if (urlStudio) {
          const text = await fetchShareText(urlStudio);
          if (text && text.trim().startsWith(STUDIO_SHARE_PREFIX)) await importStudioShare(text);
        }
      } catch (e) { console.log("[VIEWER] 自動更新に失敗(キャッシュで表示継続):", e && e.message); }
    })();
  }, [handleImported]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#071018", fontFamily: "'Noto Sans JP', Inter, sans-serif", color: C.text, paddingBottom: page === "menu" ? "0px" : "80px" }}>
      ...
      {page !== "menu" && (<button onClick={() => setShowShare(true)} aria-label="設定・共有" style={{ position: "fixed", right: "14px", bottom: "14px", zIndex: 120, width: "52px", height: "52px", borderRadius: "17px", cursor: "pointer", background: "linear-gradient(160deg, rgba(0,229,255,0.13), rgba(10,8,20,0.92))", border: `1px solid ${C.cyan}55`, color: C.cyan, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 8px 24px rgba(0,0,0,0.55), 0 0 16px ${C.cyan}22`, backdropFilter: "blur(8px)" }}><Settings size={22} strokeWidth={1.8} /></button>)}
      <Toast toast={state.toast} onDismiss={() => dispatch({ type: "TOAST", toast: null })} />
      <div style={{ background: "linear-gradient(180deg,rgba(7,16,26,0.92) 0%,transparent 100%)", padding: "16px 16px 12px", display: "flex", alignItems: "center", gap: "10px", position: "sticky", top: 0, zIndex: 50, backdropFilter: "blur(12px)" }}>
        {page !== "menu" && (
        ...
        {page === "menu" && <button onClick={() => setShowShare(true)} aria-label="設定・共有" style={{ ...B, background: "rgba(255,255,255,0.05)", border: `1px solid ${C.border}`, color: C.cyan, width: "34px", height: "34px", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Settings size={18} strokeWidth={1.8} /></button>}
      </div>
      <div key={page} style={{ position: "relative", zIndex: 1, padding: "12px 16px 0", maxWidth: "600px", margin: "0 auto", animation: "bltFadeUp 0.25s ease-out" }}>
        ...
        {page === "stats" && <StatsPage sessions={state.sessions} growth={state.growth} growthLoading={state.growthLoading} onRefreshGrowth={refreshGrowth} />}
        ...
        {page === "notes" && <PlayerNotesPage sessions={state.sessions} growth={state.growth} onRefreshGrowth={refreshGrowth} growthLoading={state.growthLoading} progress={state.growthProgress} />}
        {page === "coach" && <CoachPage sessions={state.sessions} growth={state.growth} onRefreshGrowth={refreshGrowth} growthLoading={state.growthLoading} progress={state.growthProgress} />}
        {page === "matching" && <MatchingPage sessions={state.sessions} />}
        {page === "studio" && <StudioPage onToast={(t) => dispatch({ type: "TOAST", toast: t })} />}
      </div>
      {page !== "menu" && (
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 100, display: "flex", justifyContent: "center", padding: "6px 0 10px", pointerEvents: "none" }}>
          <button onClick={() => setPage("menu")} style={{ ...B, pointerEvents: "auto", padding: "7px 26px", borderRadius: "999px", background: "rgba(8,6,15,0.92)", border: `1px solid ${C.border}`, color: C.muted, fontSize: "11px", backdropFilter: "blur(10px)", display: "flex", alignItems: "center", gap: "9px" }}>
            <span style={{ width: "34px", height: "4px", borderRadius: "2px", background: INK, display: "inline-block" }} />ホーム
          </button>
        </div>
      )}
      {showShare && <ViewerDataModal onClose={() => setShowShare(false)} onImported={handleImported} onToast={(t) => dispatch({ type: "TOAST", toast: t })} />}
    </div>
  );
}
